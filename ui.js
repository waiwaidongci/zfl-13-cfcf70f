(function (root, factory) {
  ...
  function (TidepoolCore, TidepoolState, TidepoolEcosystem, TidepoolRenderer) {
    const {
      state,
      INITIAL_BUDGET,
      CHALLENGES,
    ...
    const { applyTool, advance, startChallenge, quitChallenge, getChallengeGoalsStatus } =
      TidepoolEcosystem;
    ...
    function renderImpacts(impacts) {
      ...
      impacts.forEach((impact) => {
        ...
        const strengthDots = Array.from(
          { length: 3 },
          (_, i) => `<span class="${i < impact.strength ? "active" : ""}"></span>`
        ).join("");
    ...
    function renderFeedbackSummary(netDelta, summary) {
      refs.feedbackSummaryEl.classList.remove(
        "summary-positive",
        "summary-negative",
        "summary-neutral"
      );
    ...
    function renderFeedbackHistory() {
      ...
      state.feedbackHistory.forEach((entry) => {
        ...
        const deltaClass =
          entry.netDelta > 0 ? "delta-up" : entry.netDelta < 0 ? "delta-down" : "delta-flat";
        const deltaText =
          entry.netDelta > 0
            ? `+${entry.netDelta}`
            : entry.netDelta < 0
        ...
        const causeTags = (entry.impactLabels || [])
          .map((lbl) => {
    ...
    function updateFeedbackPanel(explainResult, _dayNum, _phase, _levelValue) {
      if (!explainResult) {
